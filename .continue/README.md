...
| **Anexos minor perdidos no Build** | 1.4.0 e 1.5.0 não têm nenhum anexo: o `concurrency: cancel-in-progress: true` do `build.yml` cancelou a construção do minor quando o push seguinte chegou, e todo mundo depois era patch, que não constrói nada de propósito. O comentário do workflow aceita que "uma versão intermediária pode ficar sem anexos" porque "o que precisa ser instalável é a mais nova" — o que não vale para **minor**, já que a mais nova costuma ser um patch e patch nunca reconstrói. `deploy-server.sh` deriva `X.Y.0` e baixaria um anexo que não existe. Resolver antes do ADR-081, que é assinar algo que não está sendo publicado | Implementation |
| **Assinatura do binário do servidor** | Implementar [ADR-081](../docs/decisions.md#adr-081--the-server-binary-is-signed-with-a-key-ci-never-holds-and-a-deploy-that-cannot-verify-changes-nothing): chave minisign separada da do atualizador, metade pública commitada, metade privada fora do repositório e fora do CI; o anexo de Release passa a sair do build local assinado, com o CI ainda compilando o binário sem assinar; `deploy-server.sh` ganha `minisign -V` contra a chave fixada, antes de qualquer coisa tocar `/usr/local/bin`, e numa falha sai diferente de zero sem parar o serviço. Depende do item acima | Samir / Implementation |
