...
> **Status:** `ACTIVE` · Last reviewed 23/09/2026, repository at `1.8.43` —
...
|---|---|---|
| **Atualização desktop** | **Remedido de fora em 23/09**: `linux-x86_64-deb.json` e `linux-x86_64-appimage.json` em `1.8.21`, `darwin-aarch64-app.json` em `1.7.21` (o macOS está atrás do Linux: cada plataforma sai do seu próprio build), e `/p/tura-notes` lista até `1.8.21`. Publicação e chave não faltam mais (a chave é `~/.config/tura-notes/updater.key`). **O que resta é o aceite, que nenhum script infere:** atualização instalada entre duas versões em macOS, AppImage, deb e rpm, e conferir que o `.deb` de `1.6.1` remove o pacote `notes` pré-`1.0.0` na máquina que ainda o tem (ADR-082). Estado completo em [updater.md](../docs/updater.md#what-is-published-measured-from-outside--23092026) | Samir |
