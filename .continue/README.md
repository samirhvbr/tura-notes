...
| **Updater: recusar antes de falhar, quando o app não pode se substituir** | O `supported()` em `apps/notes-app/src-tauri/src/updater.rs` confere o tipo de bundle e o caso pacman, e **não** confere se o aplicativo está num lugar de onde consegue se trocar. No macOS é o caso comum: rodando do `.dmg` montado ou de um caminho de App Translocation (`/private/var/folders/…/AppTranslocation/`), o `rename` do `.app` dá `EXDEV` e o plugin volta sem sequer pedir senha. Hoje o app oferece a atualização e falha depois; devia dizer antes, como já faz com o pacman. **Onde:** `supported()` passa a devolver `false` quando `current_exe()` cai sob `AppTranslocation` ou `/Volumes/`, e o `update.unsupported` ganha a frase de mover para `/Applications`. O 1.6.69 já mostra a causa real quando falha — isto é o passo seguinte, não substituto. Na fila como **R7-06** — o MinGW que isto esperava foi instalado em 23/09 | Implementation |
