...
| **Carimbo de versão deixado para trás** | Durante a rodada de 17/09 o `tauri.conf.json` ficou com a versão carimbada depois de um build, quebrando o passo `version placeholder` da execução seguinte do gate. **Não reproduzido:** nem a suíte de packaging sozinha nem o gate inteiro sujam a árvore, e os dois scripts de build restauram o placeholder na saída, inclusive em falha. A metade cosmética (o `©` reescrito como escape) era do próprio carimbador e está corrigida em 1.6.6 — o que resta é descobrir quem carimbou sem restaurar. Reproduzir antes de mexer | Implementation |
