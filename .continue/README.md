...
| **Windows intermitente no CI** | O job `rust (windows-latest)` alterna verde e vermelho em commits que não tocam o código que falha: vermelho em 1.4.3 (`the_tree_appears_in_well_under_a_second`, 2 160 diretórios em **1 243 ms** contra um teto de 1 000 ms), verde em 1.4.4, vermelho em 1.4.5 (`received_bytes_remain_pending_until_explicit_application`, `control.rs:901`, log com 3 em vez de 4). São duas falhas distintas. A primeira é um limite de relógio de parede num runner compartilhado — decidir se ADR-034 se afere por tempo de parede no CI ou por outra medida é decisão de dono, não de quem afrouxa a asserção. A segunda não tem diagnóstico e precisa de uma máquina Windows | Samir / Implementation |
