...
## 3. The twenty-seven flows, re-indexed

`ACCEPTANCE-0.1b.md`'s U1–U12 and `ACCEPTANCE-0.1c.md`'s C1–C15, with **the
...
| C15 | The update that fails | **Provoke it** — from the mounted `.dmg` on macOS, or by dismissing the password prompt on Linux | The workspace comes back as it was; the sentence names this platform's likely cause; and the updater's own error is printed under it, verbatim and untranslated |
