...
| crash | `tools/crash-save-loop`: spawns a writer against a fresh `mktemp -d`, `SIGKILL`s at random points 1000×, asserts no truncated or empty note, that every round ended by that `SIGKILL` (status 137, not the writer giving up), and that at least one write landed (since 1.8.23; `tools/tests/test_crash_loop.py` proves both refusals with stub writers) | Linux, macOS, Windows and Arch on every push to `master` and every night (`.github/workflows/crash.yml`, since 1.8.24; Linux only before). The Windows leg ends the writer with `taskkill` and is non-blocking until it passes ten nightly runs in a row (R7-14) |
...
`fixtures/large` is generated by hand with `tools/gen-large.sh` (deterministic
seed) and never committed. No CI job generates it: the two timed measurements
that read it (`performance.rs`, `search.rs`) are the owner's to run.
