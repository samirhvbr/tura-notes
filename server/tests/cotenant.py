...
# the ordering and the fail-closed behaviour; the round trip is exercised below
# against a real minisign with a throwaway key.
# The guarded FORM, and before the ordering, so that removing a step entirely
# reports what is missing instead of a `substring not found` from the `.index()`
# below. A test whose failure is a traceback is worth less than one that names
# the problem.
#
# `[ -f "$pubkey" ] || true` contains every token a presence check looks for and
# skips the verification completely, which is how the first version of this
# passed a deliberate regression. The form is what is asserted.
for required, why in [
    ('[ -f "$pubkey" ] || fail', "a missing pinned public key must refuse, not skip verification"),
    ("command -v minisign >/dev/null 2>&1 || fail", "a host without minisign must refuse, not skip verification"),
    ('curl -fsSLO "$base/$name.minisig"', "the signature is never fetched"),
    ('minisign -Vm "$name" -p "$pubkey"', "the signature is fetched and never checked"),
]:
    assert required in deploy, why

# Before `tar`, not merely before `install`: extracting an unverified archive is
# already trusting it. The first version said "before install", and a regression
# that moved verification past the extraction passed it — an assertion that
# cannot fail is decoration.
assert deploy.index("minisign -Vm") < deploy.index("tar -xzf"), \
    "an unverified archive is extracted before its signature is checked"
assert deploy.index("sha256sum -c") < deploy.index("minisign -Vm"), \
    "the signature is checked before the cheaper checksum that would fail faster"

# Refusing must not take the service down with it: the running binary keeps
# running, because a mismatch is likelier to be a publishing mistake than an
# attack and an outage would be a second failure caused by the first.
signature_failure = deploy[deploy.index("minisign -Vm"):deploy.index("tar -xzf")]
assert "systemctl" not in signature_failure, \
    "a failed signature check touches the running service"

# The real round trip: sign with a throwaway key, verify with its public half,
# and confirm a tampered byte is refused. The keys live and die in a temporary
# directory — the repository's own key is generated by the owner and its private
# half never exists here.
with tempfile.TemporaryDirectory() as temp:
    temp = pathlib.Path(temp)
    secret, public = temp / "sec.key", temp / "pub.key"
    subprocess.run(["minisign", "-G", "-p", str(public), "-s", str(secret), "-W"],
                   check=True, capture_output=True)
    artifact = temp / "notes-server-9.9.0-x86_64-linux.tar.gz"
    artifact.write_bytes(b"a binary, for the purposes of this test")
    subprocess.run(["minisign", "-Sm", str(artifact), "-s", str(secret)],
                   check=True, capture_output=True, cwd=temp)
    ok = subprocess.run(["minisign", "-Vm", str(artifact), "-p", str(public)],
                        capture_output=True, cwd=temp)
    assert ok.returncode == 0, f"a good signature did not verify: {ok.stderr}"

    # And it still refuses a changed byte, or the check above proves nothing.
    artifact.write_bytes(b"a binary, for the purposes of this tesT")
    bad = subprocess.run(["minisign", "-Vm", str(artifact), "-p", str(public)],
                         capture_output=True, cwd=temp)
    assert bad.returncode != 0, "a tampered artifact verified against the signature"

# The signing tool refuses to overwrite a key that exists, because generating
# over one in use silently invalidates every signature already published and the
# symptom appears in a deploy, on a host that is serving.
signer = ROOT / "tools/sign-server-release.sh"
assert os.access(signer, os.X_OK), "sign-server-release.sh is not executable"
with tempfile.TemporaryDirectory() as temp:
    temp = pathlib.Path(temp)
