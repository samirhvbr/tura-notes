...
- 🔒 R8-03 — **autorizado em 24/09 e travado no mesmo dia: o sudo do servidor pede senha.** A 1.9.0 foi a primeira minor depois da autorizacao. `tools/build-linux.sh --publish` roda por `ssh` `sudo -u www-data php artisan files:add` e `tools/updater-release.py` roda `sudo -u www-data mkdir/install/mv` em `public/updates/tura-notes`; medido sem mudar nada, `sudo -n -u www-data true` no b3sys responde 'a password is required'. Pergunta `q_publish_sudo` no quadro: o dono roda a cada minor com aviso meu (recomendado), regra sudoers NOPASSWD restrita, ou diretorio de updates gravavel sem sudo. O publish da 1.9.0 esta nos Atos do dono do quadro. Eu devia ter medido o sudo antes de fazer a pergunta anterior
