...
- [ ] R7-04 — **ADR-096 aceito em 24/09 ('Sim, como proposto'); o servidor entrou em 1.8.66, falta a tela.** Feito em 1.8.66: permissao `devices` (setima, nunca implicita), `GET /v1/workspaces/{w}/sync/devices` (dispositivo, rotulo da credencial dona, recibos, revogada, `yours`) e `POST …/sync/devices/{id}/revoke` (revoga a credencial dona de OUTRO dispositivo; o proprio responde 409 `own_device`, desconhecido 404, sem permissao 403; idempotente), auditado como `sync_device_revoke` com `device:<id>` como alvo; a rota de revogacao toma o lock administrativo exclusivo, como o `token revoke` do host; OpenAPI, ajuda da CLI, `security.md` §4.10 emendado, SERVER-0.5 e SYNC-0.6 atualizados. **Falta:** cliente (`notes-sync-client` remote + comandos Tauri) e a tela no painel de dispositivos, mostrando a lista so quando a credencial do app tem `devices` (403 = esconde) e *Revogar* por linha que nao seja a propria. Texto original: pergunta ao dono sobre o ADR-096
...
- [ ] R8-03 — **autorizado em 24/09: o loop publica o feed Linux a cada minor, de worktree limpo.** Resposta `q_publish_linux`: 'Sim, a cada minor (X.Y.0), de worktree limpo'. Escrito no `.loop/SCOPE.md` em 1.8.65. Proxima: a 1.9.0 — `git worktree add ../tura-notes-publish origin/master`, `tools/build-linux.sh --publish` la dentro, feed lido de fora, worktree apagado. O item fica aberto porque e recorrente; cada publicacao vira uma linha aqui
