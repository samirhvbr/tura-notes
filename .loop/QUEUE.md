...
- [ ] R6-01 — **Restaurar um draft nunca chega ao CodeMirror, e a primeira tecla apaga o texto recuperado** (`apps/notes-app/src/stores/editor.ts:180`, severidade alto, conserto pequeno). `resolveDraft` troca o documento da store por `fromOpened(opened)`, que zera `externalRev` e mantém `noteId`/`readOnly`. As duas únicas rotas que atualizam o documento do CodeMirror são o efeito de reconstrução em `Editor.tsx` (dependências `[noteId, readOnly, lineNumbersOn, wrapOn, tabSize, fontSize]`, nenhuma muda aqui) e o watcher de `externalRev` em `EditorBody`, que sai cedo porque `applied.current === externalRev` (ambos 0). A store fica com o texto do draft e a tela com o texto do disco. O banner também não some: `resolve_draft` só limpa `opened.draft` no ramo `Discard` (crates/notes-core/src/lib.rs:863-866), então o usuário clica em 'Restaurar' e não vê absolutamente nada mudar. `acceptSyncReload` (editor.ts:267) já faz o certo no mesmo arquivo: incrementa `externalRev`. **Cenário:** O app caiu com `note.md` sujo, existe draft em `.notes/drafts`. O usuário reabre, clica em 'Restaurar': nada muda na tela e o banner continua. Ele digita um caractere; o listener chama `edit(u.state.doc.toString())` com o texto do DISCO mais o caractere, o autosave de 750 ms grava isso em `note.md` e `drafts::discard` (lib.rs:756) apaga o `.draft`. O trabalho recuperado é destruído nesse ponto, sem erro e sem cópia em `conflicts/`. **Conserto:** No ramo de restauração, `externalRev: doc.externalRev + 1` em vez de zerar; mesma coisa em `resolveConflict` e `convertEol`, que hoje escapam só por acidente (um muda `readOnly`, o outro só é alcançável com o `Editor` desmontado). Vale também resetar o ref `applied` (Editor.tsx:189) quando `noteId` muda. Um teste que renderize o `Editor`, restaure e leia o documento visível pegaria — não existe `Editor.test.tsx` hoje. *(tema: O que o usuário escreveu está menos protegido no front-end do que no core)*
