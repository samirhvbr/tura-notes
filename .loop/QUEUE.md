...
- [x] R7-09 — **feito em 1.8.65 — o instrumento e o roteiro; a causa espera a execucao do dono.** Resposta de 24/09: 'Nao; eu rodo numa maquina Windows'. A assercao que falha (log do par 3 contra 4, depois de ligar `capture_new`) agora imprime cada motivo que `stage_receiver_changes` tem para nao capturar nada sem erro — publicacoes pendentes, captura em andamento, quanto da aplicacao andou, e cada arquivo do inventario com a nota que casou, se casou — via `Store::new_note_capture_diagnosis` (so em teste). Suspeita a conferir: a identidade do `new.md` vir do file ID do NTFS e casar com uma nota ja aplicada. Roteiro em `docs/OWNER-ACTS.md` §6 (clone, 20 execucoes em PowerShell, parar na primeira falha, colar o arquivo). O `ignore` fica ate o resultado. Texto original: PR de diagnostico para o job windows-latest
...
- [x] R6-33 — **feito em 1.8.32.** `tools/third-party.py` gera `THIRD-PARTY-NOTICES.md` (dependencias normais dos quatro binarios + npm de producao, MPL-2.0 nomeados, textos de licenca), instalado por `.deb`, AppImage, tarballs e AUR; `--check` no gate e CI (desatualizado ou licenca fora da allowlist falha). `NOTICE` aponta para ele
- [x] R6-34 — **feito em 1.8.33.** `updater.md` e a linha do indice remedidos de fora em 23/09 (Linux 1.8.21, macOS 1.7.21, `/p/tura-notes` ate 1.8.21), snapshot de 18/09 mantido como registro; `tools/queue-stamp.sh` no gate e CI; indice recarimbado depois de ler cada linha
- [x] R6-36 — **feito em 1.8.35.** Temporario deterministico `.notes-create-<nome>.tmp` com remove + O_EXCL, publicado por rename exclusivo (hard link na falta); a varredura do PathIndex remove os `.notes-create-*.tmp` com mais de dez minutos. O modo `--create` no crash-writer ficou de fora
- [x] R6-37 — **feito em 1.8.34.** `settle_spawn`: spawn que falha limpa `building`, e a proxima invalidacao tenta de novo, como o `content_index` ja fazia
- [x] R6-38 — **feito em 1.8.37.** Varre os bytes-fonte do code span entre as crases; golden re-abencoado lendo o diff (cada span conferido fatiando a fonte); corpus com `€` em crases simples e duplas e link em bloco cercado
- [x] R6-39 — **feito em 1.8.36.** `server_version()` extrai o primeiro X.Y.Z do `version.md`; teste de tres numeros e de extracao num documento markdown
- [x] R6-40 — **feito em 1.8.40.** Wrappers `presentation`, botao de nova nota fora do tablist, tabIndex rotativo, setas/Home/End abrem, Delete fecha, fechar fora da ordem de Tab, `aria-controls` + `role=tabpanel`; `Tabs.test.tsx`
- [x] R6-41 — **feito em 1.8.38.** A frase nas tres paginas nomeia o §2 do `SPIKE-0.0.md`; gemeos iguais
- [x] R6-42 — **feito em 1.8.39.** `ReadOnlyBanner` com a razao (`schema_ahead` com o formato, `identity_lost` com as saidas) acima do editor; tres testes
- [x] R7-12 — **feito em 1.8.43 — o instrumento, nao a causa.** CI de 1.8.15 no macOS (run 35927724141): `saved_receiver_edits_publish_without_a_remote_conflict_or_source_rewrite`, `stage_receiver_edits` -> `activity state file (exclusive)`. Depois de 1.8.12 isso so pode ser `WouldBlock`: **alguem segurava o lease naquele instante**, dentro do data dir da propria fixture, e a leitura do codigo nao acha quem (nenhum servico aninhado, nenhuma thread com o `_activity`, nenhum fork no binario). Agora `activity::acquire` devolve um `Lease` que, em build de debug, registra a pilha de quem o tomou; numa recusa imprime no stderr os detentores daquele arquivo no processo (ou 'nenhum, entao e fora do processo'), e `cargo test` mostra o stderr do teste que falha. **A proxima ocorrencia nomeia o detentor.** A linha do intermitente em `.continue/README.md` continua sendo o rastreador
- [x] R7-04 — **feito em 1.8.66 e 1.9.0 — ADR-096 construido.** Servidor em 1.8.66 (permissao `devices`, `GET …/sync/devices`, `POST …/sync/devices/{id}/revoke`, 409 `own_device`, 404, 403, idempotente, auditado como `sync_device_revoke`, lock administrativo exclusivo so nessa rota, §4.10 emendado). App em 1.9.0: `Remote::devices` (403/404 = sem lista, nao erro) e `Remote::revoke_device` (409 = `OwnDevice`, causa propria `own_device`), `Controller::devices/revoke_device` sem lock (nao escrevem nada local, como o probe), comandos `sync_control_devices`/`sync_control_revoke_device`, e o `DeviceList` no painel pareado: *Revogar* em todo dispositivo que nao e este, confirmacao no lugar (sem dialogo bloqueante) dizendo que o outro para de sincronizar ate ganhar credencial nova, e nada quando a credencial nao tem `devices`. Aceite no app instalado fica com o dono, como todo marco. Texto original: pergunta ao dono sobre o ADR-096
- [x] R7-05 — **feito em 1.8.42 (ADR-087).** Medido antes de escolher: vault de 42 MB (30 MiB de conteudo) custa ~99 ms por pagina ou fetch em release, porque cada request carrega o documento inteiro. Limites dobrados (64 MiB de conteudo, 128 MiB de estado, 20.000 revisoes) com o custo escrito ao lado, no servidor e no cliente. `GET .../sync/capacity` (Read; servidor antigo = 404 = sem aviso), perguntado uma vez por passe sem nunca falhar o passe, e o painel avisa a partir de 80%. Especificado em SYNC-0.6.md; CLAUDE/AGENTS e o indice da fila atualizados
...
- [x] R7-04 (duplicata colhida do chat em #0033) — resolvida com o item real acima: ADR-096 aceito em 24/09 e construido em 1.8.66 e 1.9.0
- [x] R7-09 (duplicata colhida do chat em #0033) — resolvida com o item real acima (1.8.65)
- 🔒 R7-08 (duplicata colhida do chat em #0033) — o item real esta acima; a fila movel espera aparelho
- 🔒 Assinatura da **1.9.0** do servidor — ato do dono (`tools/sign-server-release.sh 1.9.0`, chave privada, fora do escopo do loop). A 1.8.0 deixou de importar em 25/09: o deploy instala a minor da linha em que esta, e a linha agora e a 1.9; o deploy das 07:14 recusou a 1.9.0 por falta do `.minisig`, com a 1.7.0 no ar. A chave nao esta no desktop Linux. No quadro como OWNER_sign
- 🔒 Aceites em release instalada — ato do dono, um `ACCEPTANCE-*.md` por marco (o do 0.0 e o §2 do `SPIKE-0.0.md`)
- 🔒 Feed do macOS em 1.7.21 contra Linux 1.9.2 (medidos de fora em 25/09) — publicar exige build assinado no MacBook (ADR-092); esta maquina e Linux
...
## Rodada 8 — 24/09: o que ainda da para produzir, e o que e decisao do dono

Triagem medida em 24/09: fila em 0 abertos / 9 parados. O unico marco com trabalho e o 0.4, e quase todas as linhas do `ACCEPTANCE-0.4.md` precisam de aparelho; a que nao precisa e o polling com orcamento (`MOBILE-0.4.md` §Polling). O app desktop hoje faz poll a 5 s fixos, sem orcamento, e o aviso diz "a cada poucos segundos". 10 PRs do Dependabot abertos: o #19 (reqwest) ja estava em master e foi fechado; os outros 9 foram aplicados a mao em 1.8.50–1.8.58. O dono perguntou (24/09) por que a 1.6.100 "nao atualiza sozinha": a checagem e automatica e o print mostra ela achando a 1.8.21; a INSTALACAO e explicita por decisao (ADR-074). Instalar sozinho reverte parte do ADR-074 -> pergunta. E o feed Linux esta em 1.8.21 porque so o dono publica (`build-linux.sh --publish` nao esta no escopo) -> pergunta. Noite de 24/09: a perna Windows do crash loop passou as 1000 rodadas com `taskkill` (1 de 10).

- [x] R8-00 — **feito — os bumps em 1.8.50–1.8.58, o fechamento pelo proprio Dependabot.** Conferido as 15:08 de 24/09, com a 1.8.58 em master e o CI dos quatro SOs verde em cada uma: os 9 PRs (#21, #22, #23, #24, #25, #26, #27, #28, #29) fechados pelo Dependabot com "is up-to-date now, so this is no longer needed", 0 PR aberto, e `git ls-remote --heads` so lista `master`. Nenhum precisou de fechamento a mao. Texto original: **fechar os 9 PRs do Dependabot e apagar os branches**, cada um depois que o bump correspondente (1.8.50–1.8.58) estiver em master com CI verde. Pedido do dono em 24/09 ("pousar em master ou apagar o que ja estiver em master"). Comentario em cada PR apontando o commit que o aplicou. `git fetch -p` no fim; so `master` deve sobrar no remoto. Medido depois da 1.8.50: o Dependabot fecha o PR e apaga o branch sozinho quando o bump chega em master (#23 sumiu minutos depois); o que sobra do item e conferir o remoto no fim e fechar a mao o que ele nao fechar
- 🔒 R8-01 — **polling com orcamento: a hipotese mediu zero para agora.** O orcamento pertence a superficie do adaptador (`Caps` ou `Degraded`, contrato do `FileSystem`), e mudar essa superficie e bump Y (1.9.0) pelas regras da casa. O unico adaptador que declararia um orcamento diferente dos 5 s atuais e o SAF, que nao existe e so se verifica em aparelho (ADR-092, MacBook). No desktop sobraria so o texto do aviso ("a cada poucos segundos" -> "a cada 5 segundos"). Construir o mecanismo antes do adaptador seria infraestrutura especulativa com um produtor constante. **Entra junto com o adaptador SAF**, no mesmo bump que muda a superficie
- [x] R8-02 — **respondida em 24/09: manter como esta — e o relato era outro.** O dono escolheu 'Manter como esta' e escreveu: 'meu report e que nao esta funcionando, eu clico em instalar e da erro'. Nao era pedido de instalacao automatica; e defeito, e virou o R8-06. Texto original: atualizacao automatica, pergunta `q_autoupdate`
- 🔒 R8-03 — **respondido em 25/09 (regra sem senha, restrita) e construido em 1.9.3; espera o dono instalar o helper.** Uma linha sudoers com curinga nao seria restrita (`*` casa espaco e `../`: `files:add *` ingeriria qualquer arquivo que o www-data le, e `install … /updates/tura-notes/*` gravaria em qualquer lugar). Entao a regra nomeia um programa so, `server/cotenant/tura-publish`, que confere cada argumento, grava so no destino constante dele e nao le por symlink (testado em `server/tests/cotenant.py`); `build-linux.sh`, `build-local.sh` e `updater-release.py` usam o helper quando `sudo -n -l` diz que esta concedido, e pedem senha como antes quando nao. Instalacao: `docs/OWNER-ACTS.md` §7. Sai da fila quando o loop publicar a primeira minor sozinho. Historico: a 1.9.0 travou no sudo em 24/09; o dono publicou a 1.9.2 a mao em 25/09
- [x] R8-05 — **feito em 1.8.62 — achada a causa do intermitente de lease no macOS, e corrigida.** O instrumento do R7-12 disparou duas vezes no CI macOS de 1.8.60 (run 36040110533; 1.8.59 e 1.8.60 sem Rust, `git diff --stat 298fd8c fabd02a`, o que justificou o unico rerun, vermelho de novo): `recovery.rs:1341` e `recovery.rs:2090`, `activity state file (shared)`, '0 lease(s) no processo'. Antes de confiar, fechados dois pontos cegos do instrumento (grafia `/var` x `/private/var` -> casa por inode; mutex envenenado lia vazio -> le atraves e conta os vivos). A resposta estava certa: o detentor e um **processo sendo criado por outra thread** — o filho carrega copia de todo descritor ate o exec, o `flock` e do arquivo aberto, entao um lease solto naquela janela continua travado. No macOS toda exclusao de nota cria um (`trash` -> `osascript`/Finder), por isso so macOS. Medido no Linux com uma thread criando `/bin/true`: 274 mil recusas de um lock sem dono em 1,4 milhao de rodadas; zero em 1,3 milhao sem ela. Correcao: o lease recusado tenta de novo por 250 ms antes de reportar (o write lock ja espera 5 s), e a sonda do `forget` passa pelo mesmo caminho; um teste com thread criando processos falha nas rodadas 1-3 sem a janela. A linha do intermitente em `.continue/README.md` sai depois de uma semana de CI macOS sem ele
- [x] R8-06 — **feito em 1.8.64 — por que a 1.6.100 acha a atualizacao e nao instala.** Medido nesta maquina, que e a do relato (`.deb` 1.6.100 em `/usr/bin/notes`): feed `linux-x86_64-deb.json` em 1.8.58, `.deb` baixa (7,1 MB), assinatura confere com `minisign -V` contra a chave embutida na 1.6.100, o `infer::archive::is_deb` do plugin aceita, dependencias satisfeitas, marcador `__TAURI_BUNDLE_TYPE_VAR_DEB` no binario. E o `journalctl` nao tem **nenhum** `pkexec` vindo do Tura, contra quatro de dois outros apps Tauri (shv-ia, s-shv-term) atualizando pelo mesmo `pkexec dpkg -i` desde 21/09. Entao para antes do plugin: e a barreira do editor da 1.6.x, que a 1.7.0 corrigiu (a tomada era um instante contra o poll de 500 ms / 3 s / 15 s — cara ou coroa que a instalacao quase sempre perde, 'nothing was attempted'). A correcao nao alcanca a copia cujo instalador e o defeito: sair da 1.6.x e a mao, uma vez — `docs/OWNER-ACTS.md` §5 (ato do dono: `sudo apt install` do `.deb` verificado). `docs/updater.md` dizia, desde hoje cedo, que a 1.6.100 'instala quando pedido' — corrigido
- 🔒 R8-04 — **tirar o `continue-on-error` da perna Windows do crash loop** quando ela passar dez noites seguidas (`gh run list --workflow crash.yml`, conferir o job `windows-latest`, nao a run). 24/09: 1 de 10; 25/09: 2 de 10 (as duas noites verdes). Espera tempo, nao decisao
