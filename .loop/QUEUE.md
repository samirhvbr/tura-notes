...
- [ ] R7-11 — **o intermitente do recovery chegou discriminado: `activity state file (shared)`** (`crates/notes-core/src/activity.rs:44`). CI de 1.8.5 (run 35918936437, macOS), `edits_during_transfer_are_recaptured_without_applying_the_older_publication`, `recovery.rs:2044` (`stage_receiver_edits`): `ApplicationBlocked { cause: "timed out waiting for the activity state file (shared)" }`. Primeira ocorrencia depois de 1.7.10 separar as causas — e **nao e o write lock**, e o `try_lock_shared` do lease de atividade, que nao espera nada. O data dir e por fixture, entao nao e outro teste no mesmo diretorio; sobra (a) alguem no proprio processo segurando o exclusivo nesse instante ou (b) um erro de SO que o `map_err(|_| ...)` transforma em 'timeout'. Passo 1: separar `TryLockError::WouldBlock` de `TryLockError::Error(e)` (este vira `CoreError::io` com o kind). Passo 2: procurar quem toma o exclusivo (`inventory_using` exclusivo, `open_sync_workspace`) num caminho que o `stage_receiver_edits` percorre, e se ha caminho condicional a timestamp. Atualizar a linha do intermitente em `.continue/README.md`
